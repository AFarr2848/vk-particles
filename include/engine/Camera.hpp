...
enum fe_Camera_Movement { FORWARD, BACKWARD, LEFT, RIGHT, UP, DOWN };
...
class fe_Camera {
 public:
  ...
  fe_Camera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f),
  ...
  glm::vec3 moveCamera(fe_Camera_Movement direction, float deltaTime) {
  ...
  // systems)
  void processKeyboard(fe_Camera_Movement direction, float deltaTime) {
    Position = moveCamera(direction, deltaTime);
  }

  void moveWithCollision(fe_Camera_Movement direction,
                         float deltaTime,
    ...
    for (int i = 0; i < bounds.size(); i += 4) {
      ...
      std::cout << pointToSegmentDistance(glm::vec2(newPos.x, newPos.z), p1, p2)
                << std::endl;
      if (pointToSegmentDistance(glm::vec2(newPos.x, newPos.z), p1, p2) <
          0.1f) {
        tooClose = true;
      }
    }
  ...
  void ProcessMouseMovement(float xoffset, float yoffset) {
    ...
    glm::quat yawRot = glm::angleAxis(glm::radians(-xoffset * MouseSensitivity),
                                      glm::vec3(0.f, 1.f, 0.f));
    glm::quat pitchRot =
        glm::angleAxis(glm::radians(-yoffset * MouseSensitivity), Right);
